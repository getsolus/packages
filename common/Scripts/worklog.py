...
class GitHubPR(Listable):
    ...
    @property
    def date(self) -> datetime:
        raise datetime.fromisoformat(self._data['merged_at'])
    ...
    def labels(self) -> list[str]:
        return [l['name'] for l in self._data['labels']]
    ...
    @property
    def _url(self) -> str:
        return self._data['_links']['html']['href']
...
class Git:
    ...
    def prs(self, start: datetime, end: datetime) -> List[GitHubPR]:
        return [commit.pr for commit in self.commits(start, end, True) if commit.is_pr]
