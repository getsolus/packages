...
if [[ -n "${DISABLE_BUILD_SUCCESS_NOTIFY:-}" ]]; then
...
if [ -f "package.yml" ]; then
    ...
    git show "$(git rev-list -1 HEAD .)":./package.yml > "$TMPDIR/tmp.yml"
    TAG=$("$(git rev-parse --show-toplevel)/common/Scripts/gettag.py" "$TMPDIR/tmp.yml")
elif [ -f "pspec.xml" ]; then
    # Read the file from the latest commit in directory. Not from any potential unstaged changes
    git show "$(git rev-list -1 HEAD .)":./pspec.xml > "$TMPDIR/tmp.xml"
    TAG=$("$(git rev-parse --show-toplevel)/common/Scripts/gettag.py" "$TMPDIR/tmp.xml")
else
...
JSONPAGE="$TMPDIR/builds.json"

# Download the page
curl -sSL --fail "$BUILDSERVER_JSON_URL" -o "$JSONPAGE"
...
while true; do
    ...
    BUILD_STATUS=$(jq -r '.[] | select(.id == ($ARGS.positional[0] | tonumber)) | .status' "$JSONPAGE" --args "$BUILDID")
...
INDEX_SHA=$(curl -s --fail "$INDEX_SHA_URL")
INDEX_XZ="$TMPDIR/unstable-index.xml.xz"
INDEX_XML="$TMPDIR/unstable-index.xml"
curl -s --fail "$INDEX_XZ_URL" -o "$INDEX_XZ"
unxz "$INDEX_XZ" -f

# Downloads and extracts the new index if the sha sum has changed.
update_index_if_changed() {
    local NEW_INDEX_SHA
    NEW_INDEX_SHA=$(curl -s --fail "$INDEX_SHA_URL")
...
MAX_WAIT=90
ELAPSED=0
while ! grep -qF "${TAG}" "$INDEX_XML"; do
    if [[ $ELAPSED -ge $MAX_WAIT ]]; then
        echo "Successfully built but hasn't been found in the index yet, please manually check."
        notify-send -u low "${TAG} successfully built but hasn't been found in the index yet, please manually check." -t 0
