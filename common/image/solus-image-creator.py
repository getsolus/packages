...
def configure_boot():
    ...
    cmd = "dracut --lz4 --no-hostonly-cmdline -N --kver \"%s\" --force --add \"dmsquash-live systemd pollcdrom\" --add-drivers \"squashfs ext2 vfat msdos sr_mod sd_mod ehci_hcd uhci_hcd xhci_hcd xhci_pci ohci_hcd usb_storage usbhid dm_mod device-mapper ata_generic libata\" /live.initrd" % kernel
    run_chroot(cmd)
    try:
...
linux /kernel
initrd /initrd
options root=live:CDLABEL=%(DESC)s ro rd.luks=0 rd.md=0 quiet splash""" % { 'DESC' : label, 'TITLE': title})
    except Exception, ex:
...
label live
  ...
  kernel /boot/kernel
  append initrd=/boot/initrd.img root=live:CDLABEL=%(LABEL)s ro rd.luks=0 rd.md=0 quiet splash --
menu default
label local
